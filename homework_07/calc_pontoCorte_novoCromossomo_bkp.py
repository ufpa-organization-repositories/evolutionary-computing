...
def calc_pontoCorte(pop_pais, pop_filhos, n_pontos):
    ...
    if n_pontos == 1:
        ...
        for index, ponto in enumerate(pontosCorte):
            ...
            if index == 0:
                # x
                x1 += pai1_x[:pontosCorte[0]]
                x2 += pai2_x[:pontosCorte[0]]

                # y
                y1 += pai1_y[:pontosCorte[0]]
                y2 += pai2_y[:pontosCorte[0]]

            # ultimo
            # ajuste para numeros impar de pontos de cortes
            if index == n_pontos - 1:
                # x
                x1 += pai2_x[ponto:]
                x2 += pai1_x[ponto:]
...
calc_pontoCorte(pop_pais,pop_filhos, n_pontos=4)
print(pop_filhos)